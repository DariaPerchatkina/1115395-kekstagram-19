...
var photoRandomCreate = function (count) { // создаем функцию, которая будет генерировать случайных набор фото
  ...
  for (var i = 0; i < count; i++) { // условия работы цикла
    var url = getRandomValueArr(photos/[i].jpg); // обьявим переменную для генерации имен волшебников
    var  discription = getRandomValueArr(); // добавим  переменную для генерации рандомных цветов плащей
    var likes = getRandomValueArr(); // добавим переменную для генерации рандомных цветов глаз
    var comments = [avatar, message, name];
    photoArr.push({url: url, 
        description: discription,// создадим обьект и при помощи push добавим его в массив arr(в нашем случае это пустой массив wizardsRandom)
        likes: likes,
        comments: comments
    });
  }
